...
const double Rboard = 10000; // Resistor on board
...
double getTemperature() {
  resistance = Rboard * (1023.0 / (double) analogRead(thermistorPin) - 1.0); // resistance on thermistor
  logResistance = log(resistance);
  return 1.0 / (SHC1 + SHC2*logResistance + SHC2*pow(logResistance, 3));
}
